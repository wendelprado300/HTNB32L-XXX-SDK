...
int32_t I2C_MasterTransmit(uint32_t addr, const uint8_t *data, uint32_t num, bool xfer_pending, I2C_RESOURCES *i2c) {   
    ...
    dma_transfer_config dmaTxConfig = {(void *)data, (void *)&(i2c->reg->TDR),
    ...
    if(i2c->dma)
    {
        ...
        DMA_TransferSetup(i2c->dma->tx_ch, &dmaTxConfig);
        DMA_EnableChannelInterrupts(i2c->dma->tx_ch, DMA_EndInterruptEnable);

        I2CDEBUG("i2c tx dma start\n");
        DMA_StartChannel(i2c->dma->tx_ch);
    }
    else
    {
        ...
        while(num > i2c->ctrl->cnt)
        {
            ...
            if(timeout == 0)
            {
                ...
                GPR_SWReset(g_i2cResetClocks[instance*2]);
                GPR_SWReset(g_i2cResetClocks[instance*2+1]);
        ...
        } while((QCOM_FLD2VAL(I2C_FSR_TX_FIFO_FREE_NUM, i2c->reg->FSR) != 0x10) && (ret == ARM_DRIVER_OK) && (timeout));
...
int32_t I2C_MasterReceive(uint32_t addr, uint8_t *data, uint32_t num, bool xfer_pending, I2C_RESOURCES *i2c)
{
    ...
    dma_transfer_config dmaRxConfig = {(void *)&i2c->reg->RDR, (void *)data,
    ...
    if(i2c->dma)
    {
        ...
        DMA_TransferSetup(i2c->dma->rx_ch, &dmaRxConfig);
        DMA_EnableChannelInterrupts(i2c->dma->rx_ch, DMA_EndInterruptEnable);

        I2CDEBUG("dma configure\n");
        DMA_StartChannel(i2c->dma->rx_ch);
    }
    else
    {
        // Enable interrupts to reflect specific status
