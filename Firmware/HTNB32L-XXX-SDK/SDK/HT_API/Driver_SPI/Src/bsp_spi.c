 ...
 * Project:      SPI (SSP used for SPI) Driver for Qualcomm QCX212
 * --------------------------------------------------------------------------
...
/**
  \fn          static void SPI_DMARxConfig(void* data_in, dma_address_increment_t option, SPI_RESOURCES *spi)
  ...
  \return      void
*/
static void SPI_DMARxConfig(void* data_in, dma_address_increment_t option, SPI_RESOURCES *spi)
{
...
int32_t SPI_Transfer(const void *data_out, void *data_in, uint32_t num, SPI_RESOURCES *spi)
{
    ...
    if(spi->dma)
    {
        ...
        SPI_DMARxConfig(data_in, DMA_AddressIncrementTarget, spi);
        DMA_ChannelLoadFirstDescriptor(spi->dma->rx_ch, (void *)spi->dma->descriptor);
        DMA_StartChannel(spi->dma->rx_ch);
...
void SPI_IRQHandler(SPI_RESOURCES *spi)
{
    ...
    SPIDEBUG("enter spi irq interrupt\n");
