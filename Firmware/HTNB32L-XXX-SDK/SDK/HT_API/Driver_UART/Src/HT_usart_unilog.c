...
// End USART Interface
static ARM_USART_CAPABILITIES     HAL_USART0_GetCapabilities (void)                                                { return HAL_USART_GetCapabilities (&huart0); }
static int32_t                    HAL_USART0_Initialize      (ARM_USART_SignalEvent_t cb_event)                    { return HAL_USART_Initialize (cb_event, &huart0); }
static int32_t                    HAL_USART0_Uninitialize    (void)                                                { return HAL_USART_Uninitialize (&huart0); }
static int32_t                    HAL_USART0_PowerControl    (ARM_POWER_STATE state)                               { return HAL_USART_PowerControl (state, &huart0); }
static int32_t                    HAL_USART0_Send            (const void *data, uint32_t num)                      { return HAL_USART_Send (data, num, &huart0); }
static int32_t                    HAL_USART0_Receive         (void *data, uint32_t num)                            { return HAL_USART_Receive (data, num, &huart0); }
static int32_t                    HAL_USART0_Transfer        (const void *data_out, void *data_in, uint32_t num)   { return HAL_USART_Transfer (data_out, data_in, num, &huart0); }
static int32_t                    HAL_USART0_SendPolling     (const void *data, uint32_t num)                      { return HAL_USART_SendPolling (data, num, &huart0); }
static uint32_t                   HAL_USART0_GetTxCount      (void)                                                { return HAL_USART_GetTxCount (&huart0); }
PLAT_CODE_IN_RAM static uint32_t  HAL_USART0_GetRxCount      (void)                                                { return HAL_USART_GetRxCount (&huart0); }
static uint32_t                   HAL_USART0_GetBaudRate      (void)                                               { return HAL_USART_GetBaudRate (&huart0); }
static int32_t                    HAL_USART0_Control         (uint32_t control, uint32_t arg)                      { return HAL_USART_Control (control, arg, &huart0); }
static ARM_USART_STATUS           HAL_USART0_GetStatus       (void)                                                { return HAL_USART_GetStatus (&huart0); }
static int32_t                    HAL_USART0_SetModemControl (ARM_USART_MODEM_CONTROL control)                     { return HAL_USART_SetModemControl (control, &huart0); }
static ARM_USART_MODEM_STATUS     HAL_USART0_GetModemStatus  (void)                                                { return HAL_USART_GetModemStatus (&huart0); }
PLAT_CODE_IN_RAM       void       HAL_USART0_IRQHandler      (void)                                                {        HAL_USART_IRQHandler (&huart0); }

ARM_DRIVER_USART Driver_USART0 = {
    ARM_USART_GetVersion,
    HAL_USART0_GetCapabilities,
    HAL_USART0_Initialize,
    HAL_USART0_Uninitialize,
    HAL_USART0_PowerControl,
    HAL_USART0_Send,
    HAL_USART0_Receive,
    HAL_USART0_Transfer,
    HAL_USART0_GetTxCount,
    HAL_USART0_GetRxCount,
    HAL_USART0_Control,
    HAL_USART0_GetStatus,
    HAL_USART0_SetModemControl,
    HAL_USART0_GetModemStatus,
    HAL_USART0_GetBaudRate,
    HAL_USART0_SendPolling
};
...
static int32_t                    HAL_UNILOG_USART1_Send            (const void *data, uint32_t num)                      { return HAL_USART_Send (data, num, &huart1); }
static int32_t                    HAL_UNILOG_USART1_Receive         (void *data, uint32_t num)                            { return HAL_USART_Receive (data, num, &huart1); }
static int32_t                    HAL_UNILOG_USART1_Transfer        (const void *data_out, void *data_in, uint32_t num)   { return HAL_USART_Transfer (data_out, data_in, num, &huart1); }
static int32_t                    HAL_UNILOG_USART1_SendPolling     (const void *data, uint32_t num)                      { return HAL_USART_SendPolling (data, num, &huart1); }
...
PLAT_CODE_IN_RAM       void       HAL_UNILOG_USART1_IRQHandler      (void)                                                {        HAL_USART_IRQHandler (&huart1); }
...
static int32_t                    HAL_UNILOG_USART2_Send            (const void *data, uint32_t num)                      { return HAL_USART_Send (data, num, &huart2); }
static int32_t                    HAL_UNILOG_USART2_Receive         (void *data, uint32_t num)                            { return HAL_USART_Receive (data, num, &huart2); }
static int32_t                    HAL_UNILOG_USART2_Transfer        (const void *data_out, void *data_in, uint32_t num)   { return HAL_USART_Transfer (data_out, data_in, num, &huart2); }
static int32_t                    HAL_UNILOG_USART2_SendPolling     (const void *data, uint32_t num)                      { return HAL_USART_SendPolling (data, num, &huart2); }
...
PLAT_CODE_IN_RAM       void       HAL_UNILOG_USART2_IRQHandler      (void)                                                {        HAL_USART_IRQHandler (&huart2); }
