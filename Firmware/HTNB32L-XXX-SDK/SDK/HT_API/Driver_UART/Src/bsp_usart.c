...
PLAT_CODE_IN_RAM static uint32_t USART_GetInstanceNumber(USART_RESOURCES *usart)
{
...
PLAT_CODE_IN_RAM static void USART_DmaUpdateRxConfig(USART_RESOURCES *usart, uint32_t targetAddress, uint32_t num)
{
...
int32_t USART_Send(const void *data, uint32_t num, USART_RESOURCES *usart)
{
    ...
    if(usart->dma_tx)
    {
        ...
        else
        {
            ...
            DMA_TransferSetup(usart->dma_tx->channel, &dmaTxConfig);
            DMA_EnableChannelInterrupts(usart->dma_tx->channel, DMA_EndInterruptEnable);
            DMA_StartChannel(usart->dma_tx->channel);
        }
...
PLAT_CODE_IN_RAM int32_t USART_GetRxCount(USART_RESOURCES *usart)
{
    if (!(usart->info->flags & USART_FLAG_CONFIGURED))
        return 0U;
...
PLAT_CODE_IN_RAM void USART_IRQHandler (USART_RESOURCES *usart)
{
#ifdef PM_FEATURE_ENABLE
    uint32_t instance;
#endif
    ...
    iir_reg = usart->reg->IIR;

#ifdef PM_FEATURE_ENABLE
    g_usartDataBase[instance].iir_reg = iir_reg;
    ...
    if ((iir_reg & USART_IIR_INT_PENDING_Msk) == 0U)
    {
        if((iir_reg & USART_INT_TYPE_MSK) == USART_RLS_INT)
        {
        ...
        else if((iir_reg & USART_INT_TYPE_MSK) == USART_RDA_INT)
        {
#ifdef PM_FEATURE_ENABLE
        ...
        else if((iir_reg & USART_INT_TYPE_MSK) == USART_CTI_INT)
        {
#ifdef PM_FEATURE_ENABLE
            // loop backmode is enabled for autobaud rate delay
            if(usart->reg->MCR == 0x10)
            {
                usart->reg->MFCR = 0;
                ...
                usart->reg->MCR = 0;
                usart->reg->ADCR = 0x3;
                usart->reg->MFCR = g_usartDataBase[instance].backup_registers.MFCR;
            ...
            if(usart->reg->ADCR & USART_ADCR_AUTO_BAUD_INT_EN_Msk)
            {
                ...
                usart->reg->MFCR &= ~USART_MFCR_UART_EN_Msk;
                usart->reg->LCR |= USART_LCR_ACCESS_DIVISOR_LATCH_Msk;
                usart->reg->MFCR = ((usart->reg->MFCR & ~USART_MFCR_PRESCALE_FACTOR_Msk) | QCOM_VAL2FLD(USART_MFCR_PRESCALE_FACTOR, 0));
        ...
        if(((iir_reg & USART_INT_TYPE_MSK) == USART_TFE_INT) || \
...
PLAT_CODE_IN_RAM static uint32_t  USART0_GetRxCount      (void)                                                { return USART_GetRxCount (&USART0_Resources); }
...
PLAT_CODE_IN_RAM       void       USART0_IRQHandler      (void)                                                {        USART_IRQHandler (&USART0_Resources); }
...
PLAT_CODE_IN_RAM static uint32_t  USART1_GetRxCount      (void)                                                { return USART_GetRxCount (&USART1_Resources); }
...
PLAT_CODE_IN_RAM       void       USART1_IRQHandler      (void)                                                {        USART_IRQHandler (&USART1_Resources); }
...
PLAT_CODE_IN_RAM static uint32_t  USART2_GetRxCount      (void)                                                { return USART_GetRxCount (&USART2_Resources); }
...
PLAT_CODE_IN_RAM       void       USART2_IRQHandler      (void)                                                {        USART_IRQHandler (&USART2_Resources); }
