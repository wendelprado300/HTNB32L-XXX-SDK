...
typedef struct DlPduBlock_Tag
{
    ...
    */
	...
	UINT32      bNotLwipMem : 1;   //whether this block memory is not allocated in LWIP, in fact suggest to involve a memType;
    UINT32      magicByte : 8;
...
#define DL_PDU_BLOCK_SET_BASICS(dlpdu, mtype, buf, len, bpbuf, next)   \
              do\
              {\
                  ((DlPduBlock*)(dlpdu))->magicByte   = PS_PDU_MAGIC_BYTE;\
