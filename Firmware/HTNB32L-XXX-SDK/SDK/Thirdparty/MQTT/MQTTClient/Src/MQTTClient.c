...
// static volatile MessageData* subs_buf = NULL;

// void SubscribeCallback(MessageData *output) {
//     print_uart(HT_UART1, "\nCallback: ");
//     subs_buf = output;
//     print_uart(HT_UART1, subs_buf->message->payload);
//     print_uart(HT_UART1, "\n");
// }
...
// void mqtt_demo_onenet(void) {
    ...
    // ---------- TENTATIVA DE CHAMAR HT_MQTT_API (FAIL) -------------
    // print_uart(HT_UART1, "\nmqtt_demo_onenet\n");
    ...
    // print_uart(HT_UART1, "Init\n");
    // HT_MQTT_Init(40000, mqttSendbuf, 100, mqttReadbuf, 100);

    // if(HT_MQTT_Connect("3.124.62.105", HT_MQTT_PORT, HT_MQTT_SEND_TIMEOUT, HT_MQTT_RECEIVE_TIMEOUT, 
    //         "SIP", "poc", "HTmicron", HT_MQTT_VERSION, HT_MQTT_KEEP_ALIVE_INTERVAL)) {
        
    //     print_uart(HT_UART1, "HT_MQTT_Connect error!\n");
    // }

    // print_uart(HT_UART1, "MQTT Connect Success!\n");

    // uint8_t output[10];
    // MQTTClient *HT_MQTTClient;
    // print_uart(HT_UART1, "Subscribing...\n");
    // // HT_MQTT_Subscribe("mqtt_htnbat", QOS0, output);
    // HT_MQTTClient = HT_GET_MQTTClient();
    // MQTTSubscribe(HT_MQTTClient, "mqtt_htnbat", QOS0, HT_MQTT_SubscribeCallback);
    // print_uart(HT_UART1, "Subscribe done\n");
    ...
    // uint8_t *mqttSendbuf = NULL;
    // uint8_t *mqttReadbuf = NULL;
    ...
    // HT_MQTT_Init(&mqttClient, &mqttNetwork, 40000, mqttSendbuf, 1000, mqttReadbuf, 1000);
    ...
    // HT_MQTT_Connect(&mqttClient, &mqttNetwork, "18.197.95.2", HT_MQTT_PORT, HT_MQTT_SEND_TIMEOUT, HT_MQTT_RECEIVE_TIMEOUT,
    //                 "SIP", "poc", "HTmicron", HT_MQTT_VERSION, HT_MQTT_KEEP_ALIVE_INTERVAL);
    // connectData.MQTTVersion = 4;
    // connectData.clientID.cstring = "SIP";
    // connectData.username.cstring = "poc";
    // connectData.password.cstring = "HTmicron";
    // connectData.keepAliveInterval = 120;
    // // HT_TRACE(UNILOG_MQTT, mqtt_hh00, P_SIG, 0, "mqtt_demo........");
    ...
    // else
    // {
    //     if ((NetworkConnect(&mqttNetwork, "3.124.62.105", 1883)) != 0)
    //     {
    //         mqttClient.keepAliveInterval = connectData.keepAliveInterval;
    //         mqttClient.ping_outstanding = 1;
    //     }
    //     else
    //     {
    //         // HT_TRACE(UNILOG_MQTT, mqtt_hh01, P_SIG, 0, "mqtt_demo....1....");
    ...
    //         else
    //         {
    //             // HT_TRACE(UNILOG_MQTT, mqtt_hh02, P_SIG, 0, "mqtt_demo....2....");
    ...
    // print_uart(1, "\nSubscribing...\n");
    // HT_MQTT_Subscribe(&mqttClient, "mqtt_htnbat", QOS0);
    // MQTTSubscribe(&mqttClient, "mqtt_htnbat", QOS0, SubscribeCallback);
    // print_uart(1, "Subs ok\n");
    ...
    // {
        ...
        // sprintf(mqtt_payload,"Dale Grêmio");
        // // printx(mqtt_payload);
        ...
        // // HT_TRACE(UNILOG_MQTT, mqtt_hh03, P_SIG, 0, "mqtt_demo....3....");
        
        // // printx("Message main: ");
        // // printx(message.payload);
        // // printx("\n");

        // print_uart(1, "Publish...\n");
        // MQTTPublish(&mqttClient, "mqtt_htnbat", &message);
        // printx("ok\n");

    //     print_uart(1, "Publishing...\n");
    //     memset(mqtt_payload,'\0', sizeof(mqtt_payload));
    //     sprintf(mqtt_payload,"HT publish");
    //     len = strlen(mqtt_payload);
    //     HT_MQTT_Publish(&mqttClient, "mqtt_htnbat", (uint8_t *)mqtt_payload, len, QOS0, 0, 0, 0);
    //     osDelay(15000);
    // }
    /* do not return */
// }
...
// //     /* Qualcomm  ec_smoke     a1xFDTv3InR  sWZtNMYkODMxvauyaSiGeJrVEp9jZ4Tg  8964 ---"Qualcomm|securemode=3,signmethod=hmacsha1,timestamp=8964|" "deviceGrape&a1fsx061r0x" */
// //     /* Qualcomm  deviceGrape  a1fsx061r0x  WLar6NunAcCJ0aZHbaNw4eQwdsYVKyC9  8964 ---"Qualcomm|securemode=3,signmethod=hmacsha1,timestamp=8964|" "deviceGrape&a1fsx061r0x" */
...
// //     snprintf(hmac_source, sizeof(hmac_source), "clientId%s" "deviceName%s" "productKey%s" "timestamp%s", "Qualcomm", "qc_smoke", "a1xFDTv3InR", "8964");
...
// //     sprintf(ali_clientID,"%s|securemode=3,signmethod=hmacsha1,timestamp=8964|", "Qualcomm");
// //     sprintf(ali_username,"%s&%s","qc_smoke","a1xFDTv3InR");
...
// //     connectData.MQTTVersion = 4;
// //     connectData.keepAliveInterval = 120;
// //     HT_TRACE(UNILOG_MQTT, mqtt_hh001, P_SIG, 0, "mqtt_demo........");
// //     mqttSendbuf = malloc(1000);
// //     mqttReadbuf = malloc(1000);
// //     memset(mqttSendbuf, 0, 1000);
// //     memset(mqttReadbuf, 0, 1000);
...
// //         else
// //         {
// //             HT_TRACE(UNILOG_MQTT, mqtt_hh012, P_SIG, 0, "mqtt_demo....1....");
// //             if ((MQTTConnect(&mqttClient, &connectData)) != 0)
// //             {
// //                 mqttClient.ping_outstanding = 1;
// //             }
// //             else
// //             {
// //                 HT_TRACE(UNILOG_MQTT, mqtt_hh023, P_SIG, 0, "mqtt_demo....2....");
// //                 mqttClient.ping_outstanding = 0;
// //             }
// //         }
// //         if(mqttClient.ping_outstanding == 0)
...
// //         HT_TRACE(UNILOG_MQTT, mqtt_hh034, P_SIG, 0, "mqtt_demo....3....");

// //         MQTTPublish(&mqttClient, "a1xFDTv3InR/qc_smoke/user/get", &message);
