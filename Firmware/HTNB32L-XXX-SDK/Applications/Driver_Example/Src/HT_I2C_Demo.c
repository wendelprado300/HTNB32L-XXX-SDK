...
static uint8_t tx_buffer[TX_BUFFER_SIZE] = {"HelloWorld"};
...
void HT_I2C_App(void) {
    ...
    while (1) {
        ...
        HT_I2C_MasterTransmit(SLAVE_I2C_ADDRESS, tx_buffer, strlen((char *)tx_buffer));

        print_uart("Waiting for response...\n");
        
        do {
            HT_I2C_MasterReceive(SLAVE_I2C_ADDRESS, rx_buffer, RX_BUFFER_SIZE);
        } while (rx_buffer[0] == '\0');
